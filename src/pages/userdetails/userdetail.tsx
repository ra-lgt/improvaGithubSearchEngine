...
<div className="flex flex-col md:flex-row gap-4 m-5">
  {/* User Details Card */}
  <Card.Root className="w-full md:w-1/2">
    {loading ? (
      ...
      <Card.Body gap="2">
        ...
        <Card.Description>
          <Text>GitHub Staff: {userDetails?.site_admin ? "Yes" : "No"}</Text>
        </Card.Description>
        <Card.Description>
  ...
  {/* User Events Card */}
  <Card.Root className="w-full md:w-1/2 max-h-screen overflow-y-auto">
    {loading ? (
      <CardLoader count={1} lineCount={10} />
    ) : (
      <Card.Body gap="2">
        <Card.Title className="sticky top-0 z-10 py-2 bg-white dark:bg-gray-900 shadow-sm">
          User Events
        </Card.Title>
                ...
                <div className="flex items-start justify-between gap-4 px-4 py-3 bg-white dark:bg-gray-900 shadow-md rounded-xl transition hover:shadow-lg">
                  ...
                  <div className="flex-1 text-sm text-gray-800 dark:text-gray-200">
                    <span className="font-medium">
                      {event?.actor?.display_login} {event?.payload?.action}
                    </span>{" "}
                    the repository
                    <a
